                ...
                namespace FaceAPI
                {
                    public class DocumentVerificationHandler
                    {
                        ...
                        public async Task GetTextAsync(ComputerVisionClient computerVision, string operationLocation)
                        {
                            ...
                            {
                                //Calling LUIS
                                var client = new RestClient(ConfigurationManager.AppSettings["LUIS_EndPoint"]+ "luis/v2.0/apps/" + ConfigurationManager.AppSettings["LUIS_AppID"] + "?verbose=true&timezoneOffset=-360&subscription-key=" + ConfigurationManager.AppSettings["LUIS_Key"] + "&q=" + OCRList[j]); var request = new RestRequest(Method.GET);
